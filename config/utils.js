...
function createGetReportRequestParams(accessToken){
    ...
    var url = keys.pib.apiUrl + 'v1.0/myorg/groups/' + keys.pib.workspaceId + '/reports/' + keys.pib.reportId;
    ...
    let promise = () => { return new Promise(
        (resolve, reject) => {
            request(url, options,function (error, response, body){
                ...
                if(body == ""){
                    console.log('error: no report with id: ' + keys.pib.reportId + " in group with id: " + keys.pib.workspaceId);
                    reject('error: no report with id: ' + keys.pib.reportId + " in group with id: " + keys.pib.workspaceId);
                }
                try{
...
async function sendGetDatasetRequestAsync(token, datasetId){
    ...
    var url = keys.pib.apiUrl + 'v1.0/myorg/groups/' + keys.pib.workspaceId + '/datasets/' + datasetId;
